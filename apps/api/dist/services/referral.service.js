 ...
 */
export async function getMyReferralInfo(distributorId) {
 ...
 */
export async function regenerateReferralCode(distributorId) {
 ...
 */
export async function getDistributorByReferralCode(code) {
    if (!code || code.trim().length === 0) {
        return null;
    }
    const { data, error } = await supabase.rpc('validate_referral_code', {
