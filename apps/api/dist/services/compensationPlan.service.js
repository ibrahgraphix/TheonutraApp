...
export async function listActiveStatusRanks() {
    const { data, error } = await supabase
        .from('active_status_ranks')
        .select('*')
        .order('level_order', { ascending: true });
    if (error)
        throw new ApiError(500, `Failed to fetch active status ranks: ${error.message}`);
    return (data ?? []).map((r) => ({
...
export async function listLeadershipRanks() {
    const { data, error } = await supabase
        .from('leadership_ranks')
        .select('*')
        .order('level_order', { ascending: true });
    if (error)
        throw new ApiError(500, `Failed to fetch leadership ranks: ${error.message}`);
    return (data ?? []).map((r) => ({
        id: r.id, name: r.name, level_order: r.level_order,
 ...
 */
async function calculatePPV(distributorId, month) {
    ...
    if (ordersError)
        throw new ApiError(500, `Failed to calculate PPV from orders: ${ordersError.message}`);
    let ppv = 0;
    ...
    const { data: sales, error: salesError } = await supabase
        ...
        .lt('created_at', endStr);
    if (salesError)
        throw new ApiError(500, `Failed to calculate PPV from customer sales: ${salesError.message}`);
    for (const sale of sales ?? []) {
        for (const item of sale.customer_sale_items ?? []) {
 ...
 */
async function calculateCGV(distributorId, month) {
 ...
 */
function resolveActiveStatusRank(ranks, ppv, cgv) {
 ...
 */
export async function getDistributorCompensationSnapshot(distributorId, month) {
...
async function countQualifiedDownlineLeaders(distributorId) {
    const { data: directRecruits, error } = await supabase
        .from('profiles')
        .select('id, is_active, active_status_rank_id')
        .eq('referred_by', distributorId);
    if (error)
        throw new ApiError(500, `Failed to fetch downline: ${error.message}`);
    ...
    const { data: lRankRow, error: lRankError } = await supabase
        ...
        .eq('name', 'L')
        .single();
    if (lRankError || !lRankRow)
        throw new ApiError(500, 'L rank not found in active_status_ranks');
    const lLevelOrder = lRankRow.level_order;
    const rankIds = [...new Set(directRecruits.map((r) => r.active_status_rank_id).filter(Boolean))];
    if (rankIds.length === 0)
        return 0;
    const { data: ranks, error: ranksError } = await supabase
        ...
        .in('id', rankIds);
    if (ranksError)
        throw new ApiError(500, `Failed to fetch rank levels: ${ranksError.message}`);
 ...
 */
async function resolveLeadershipRank(distributorId, ppv) {
 ...
 */
function calculateOPB(cgv, ppv, opbPercent) {
 ...
 */
export async function runMonthlyRequalification(period) {
    const { data: distributors, error } = await supabase
        ...
        .eq('is_active', true);
    if (error)
        throw new ApiError(500, `Failed to fetch distributors: ${error.message}`);
    let processed = 0;
    let demoted = 0;
    let opbGenerated = 0;
    for (const dist of distributors ?? []) {
        try {
            ...
            const wasDemoted = dist.active_status_rank_id !== null && dist.active_status_rank_id !== activeStatusRank.id;
            await supabase
                .from('profiles')
                .update({
            ...
            if (wasDemoted) {
                demoted++;
                try {
                    await notificationService.createNotification(dist.id, 'system', 'Rank Updated', `Your rank has been updated to ${activeStatusRank.name} based on this month's performance.`, { new_rank: activeStatusRank.name });
                }
                catch {
 ...
 */
export async function approveOPBBonus(opbId, staffId) {
    const { data: opb, error: fetchError } = await supabase
        ...
        .single();
    if (fetchError || !opb)
        throw new ApiError(404, 'OPB bonus not found');
    if (opb.status !== 'pending')
        throw new ApiError(422, 'OPB bonus is not pending');
    const { error: updateError } = await supabase
        .from('opb_bonuses')
        .update({ status: 'approved', approved_by: staffId, approved_at: new Date().toISOString() })
        .eq('id', opbId);
    if (updateError)
        throw new ApiError(500, `Failed to approve OPB bonus: ${updateError.message}`);
    ...
    try {
        await notificationService.createNotification(opb.distributor_id, 'system', 'OPB Bonus Paid', `Your Organisation Performance Bonus of ${Number(opb.bonus_amount).toFixed(2)} for ${opb.period} has been paid.`, { period: opb.period, amount: opb.bonus_amount });
    }
    catch {
        // ignore
    }
}
export async function rejectOPBBonus(opbId, staffId) {
    const { error } = await supabase
        ...
        .eq('status', 'pending');
    if (error)
        throw new ApiError(500, `Failed to reject OPB bonus: ${error.message}`);
}
export async function listPendingOPBBonuses() {
    const { data, error } = await supabase
        .from('opb_bonuses')
    ...
    if (error)
        throw new ApiError(500, `Failed to fetch pending OPB bonuses: ${error.message}`);
    return data ?? [];
}
//# sourceMappingURL=compensationPlan.service.js.map