 ...
 */
export async function calculateTeamBonus(distributorId, period) {
    ...
    for (const rate of rates) {
      ...
      `)
        ...
        for (const sale of customerSales ?? []) {
            const items = sale.customer_sale_items;
            for (const item of items ?? []) {
 ...
 */
export async function runTeamBonusBatch(period, staffId) {
    ...
    for (const distributor of distributors ?? []) {
        ...
        for (const levelBreakdown of summary.breakdown) {
            if (levelBreakdown.bonusAmount <= 0)
                continue;
            ...
            try {
                await notificationService.notifyTeamBonusEarned(distId, levelBreakdown.bonusAmount, period);
            }
            catch (notifError) {
 ...
 */
export async function getTeamBonusHistory(distributorId, page = 1, limit = 20) {
 ...
 */
export async function getTeamBonusRates() {
    const { data, error } = await supabase
        .from('team_bonus_rates')
        .select(`
      id,
