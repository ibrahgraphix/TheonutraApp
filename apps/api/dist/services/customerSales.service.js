 ...
 */
export async function logCustomerSale(distributorId, input) {
    ...
    const productMap = new Map(products.map((p) => [
        ...
        {
            pv: Number(p.pv ?? 0),
            price: p.product_prices[0]?.price,
            distributorPrice: p.product_prices[0]?.distributor_price,
        },
    ]));
 ...
 */
export async function listMyCustomerSales(distributorId, page = 1, limit = 20) {
    ...
    const sales = (data ?? []).map((row) => ({
        ...
        items: row.customer_sale_items.map((item) => ({
            id: item.id,
            customerSaleId: item.customer_sale_id,
            productId: item.product_id,
            quantity: Number(item.quantity),
 ...
 */
export async function getMyCustomerSalesSummary(distributorId, month) {
    const now = new Date();
    let startStr;
    let endStr;
    if (month) {
...
//# sourceMappingURL=customerSales.service.js.map