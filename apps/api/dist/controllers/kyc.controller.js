 ...
 */
export async function getMyKycHandler(req, res, next) {
    try {
        if (!req.user) {
            throw new ApiError(401, 'Unauthorized');
        }
 ...
 */
export async function listPendingKycHandler(req, res, next) {
    try {
 ...
 */
export async function getKycSubmissionHandler(req, res, next) {
    try {
        const { id } = req.params;
...
export async function reviewKycHandler(req, res, next) {
    try {
        ...
        const input = req.body;
        await kycService.reviewKyc(req.user.id, id, input);
