-- ============================================================
-- Theonutra: Wallet & Withdrawal DB Setup
-- Run this in Supabase SQL Editor (Project → SQL Editor → New query)
-- It is fully idempotent — safe to re-run multiple times.
-- ============================================================

-- 1. wallet_settings (min withdrawal / fee)
...
CREATE INDEX IF NOT EXISTS idx_wallet_tx_distributor ON wallet_transactions(distributor_id);
...
CREATE INDEX IF NOT EXISTS idx_wr_distributor ON withdrawal_requests(distributor_id);
...
DROP POLICY IF EXISTS wt_self_select             ON wallet_transactions;
CREATE POLICY wt_self_select ON wallet_transactions FOR SELECT USING (auth.uid() = distributor_id);
DROP POLICY IF EXISTS wt_staff_all               ON wallet_transactions;
CREATE POLICY wt_staff_all   ON wallet_transactions FOR ALL    USING (public.is_staff());

DROP POLICY IF EXISTS wr_self_select             ON withdrawal_requests;
CREATE POLICY wr_self_select ON withdrawal_requests FOR SELECT USING (auth.uid() = distributor_id);
DROP POLICY IF EXISTS wr_self_insert             ON withdrawal_requests;
CREATE POLICY wr_self_insert ON withdrawal_requests FOR INSERT WITH CHECK (auth.uid() = distributor_id);
DROP POLICY IF EXISTS wr_staff_all               ON withdrawal_requests;
CREATE POLICY wr_staff_all   ON withdrawal_requests FOR ALL    USING (public.is_staff());

-- 6. Trigger: auto-credit wallet when a commission row is inserted
...
BEGIN
  ...
  INSERT INTO public.wallet_transactions (
    distributor_id, type, source_type, source_id, amount, balance_after, created_at
  ) VALUES (
    NEW.beneficiary_id, 'credit', v_source_type, NEW.id, NEW.amount, v_balance, now()
  );
...
-- 7. RPC: create_withdrawal_request (atomic debit + insert)
...
BEGIN
  ...
  INSERT INTO public.withdrawal_requests (
    distributor_id, amount, method, payout_details, status, requested_at
  ) VALUES (
    p_distributor_id, p_amount, p_method, p_payout_details, 'pending', now()
...
-- 8. RPC: approve_withdrawal (status → approved, write debit ledger record)
...
BEGIN
  ...
  SELECT balance INTO v_balance FROM public.wallets WHERE distributor_id = v_distributor_id;

  INSERT INTO public.wallet_transactions (
    distributor_id, type, source_type, source_id, amount, balance_after, created_at
  ) VALUES (
    v_distributor_id, 'debit', 'withdrawal', p_request_id, v_amount, v_balance, now()
  );
END;
$$;

-- 9. RPC: reject_withdrawal (status → rejected, refund reserved amount)
...
BEGIN
  ...
  UPDATE public.withdrawal_requests
  SET status = 'rejected', reviewed_by = p_reviewed_by, reviewed_at = now(), notes = p_notes
  WHERE id = p_request_id;
