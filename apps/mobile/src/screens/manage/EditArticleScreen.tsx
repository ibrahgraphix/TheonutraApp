...
type NavigationProp = NativeStackNavigationProp<ManageStackParamList, 'EditArticle'>;
type ScreenRoute = RouteProp<ManageStackParamList, 'EditArticle'>;
...
export function EditArticleScreen() {
  ...
  useEffect(() => {
    getArticleById(articleId).then((article) => {
      if (article) {
        reset({ title: article.title, summary: article.summary, content: article.content });
  ...
  const onSubmit = handleSubmit(async (data) => {
    ...
    try {
      ...
      Alert.alert('Error', err instanceof Error ? err.message : 'Could not update article.');
    } finally {
      setSaving(false);
    }
  });
  ...
  if (loading) {
    return (
      <View style={styles.container}>
        <ShopHeader onBack={() => navigation.goBack()} title="Edit Article" />
